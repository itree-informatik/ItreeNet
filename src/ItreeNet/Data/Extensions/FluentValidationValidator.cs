...
public class ItreeFluentValidationValidator : ComponentBase
{
    ...
    [Parameter] public bool StrictMode { get; set; } = true;
    ...
    protected override void OnInitialized()
    {
        ...
        {
            throw new InvalidOperationException($"{nameof(ItreeFluentValidationValidator)} requires a cascading " +
                                                $"parameter of type {nameof(EditContext)}. For example, you can use {nameof(ItreeFluentValidationValidator)} " +
                                                $"inside an {nameof(EditForm)}.");
        }

        CurrentEditContext.AddFluentValidation(ServiceProvider, DisableAssemblyScanning, Validator, StrictMode, this);
    }
}