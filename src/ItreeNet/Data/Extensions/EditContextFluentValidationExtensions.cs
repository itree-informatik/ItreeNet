...
public static class EditContextFluentValidationExtensions
{
    ...
    public static void AddFluentValidation(this EditContext editContext, IServiceProvider serviceProvider, bool disableAssemblyScanning, IValidator? validator, bool strictMode, ItreeFluentValidationValidator fluentValidationValidator)
    {
        ...
        editContext.OnFieldChanged +=
            async (_, eventArgs) => await ValidateField(editContext, messages, eventArgs.FieldIdentifier, serviceProvider, disableAssemblyScanning, validator, strictMode);
    }
    ...
    private static async Task ValidateModel(EditContext editContext,
        ...
        ItreeFluentValidationValidator fluentValidationValidator,
        ...
        bool strictMode = true)
    {
        ...
        if (validator is not null)
        {
            if (!strictMode)
            {
                var fieldIdentifierModel = fieldIdentifier.Model.ToString();
                var editContextModel = editContext.Model.ToString();

                if (fieldIdentifierModel != editContextModel)
                    return;
            }
    ...
    private static IValidator? GetValidatorForModel(IServiceProvider serviceProvider, object model, bool disableAssemblyScanning)
    {
        ...
        {
            Log.Debug(ex, "Could not resolve validator for {ModelType} from DI", model.GetType().Name);
        }
        ...
        foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies().Where(i => i.FullName is not null && !ScannedAssembly.Contains(i.FullName)))
        {
            ...
            {
                Log.Debug(ex, "Could not scan assembly {Assembly} for validators", assembly.FullName);
            }
